...
def interp(tree, lookup, use_z3=False):
    ...
    if op in ('add', 'sub', 'mul', 'div', 'shl', 'shr'):  # Binary operators.
        lhs = interp(tree.children[0], lookup, use_z3)
        rhs = interp(tree.children[1], lookup, use_z3)
        if op == 'add':
            return lhs + rhs
        elif op == 'sub':
            return lhs - rhs
        ...
        cond = interp(tree.children[0], lookup, use_z3)
        true = interp(tree.children[1], lookup, use_z3)
        false = interp(tree.children[2], lookup, use_z3)
...
def z3_expr(tree, vars=None):
    ...
    return interp(tree, get_var, use_z3=True), vars
