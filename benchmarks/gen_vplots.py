...
results_raw = pd.read_csv('vresults.csv')
results = results_raw.pivot(index=['benchmark', 'label'], columns='run', values='result').replace({'missing': 'NaN', 'timeout': 'NaN'}).astype("float").reset_index()

results_raw

results['benchmark_name'] = results['benchmark'].apply(lambda s: (Path("..") / s).stem) + '/' + results['label']
results['benchmark_group'] = results['benchmark'].apply(lambda s: (Path("..") / s).parts[-2])
speedups = []
for opt in ['worklist', 'smart']:
...
for name, group in results.groupby('benchmark_group'):
    ...
    p = figure(x_range=benches, title=f"Speedups for {name} Benchmarks",
...
show(gridplot(plots, sizing_mode="scale_width"))
# -
