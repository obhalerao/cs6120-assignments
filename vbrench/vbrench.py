...
def get_results(strings, extract_re):
...
def vbrench(config_path, files, jobs):
    ...
    with futures.ThreadPoolExecutor(max_workers=jobs) as pool:
        ...
        writer.writerow(['benchmark', 'label', 'run', 'result'])
        for fn in files:
            first_out = None
            for name in config['runs']:
                ...
                for label, fom in results:
                    writer.writerow([
                        bench,
                        label,
                        name,
                        status if status else fom,
                    ])
